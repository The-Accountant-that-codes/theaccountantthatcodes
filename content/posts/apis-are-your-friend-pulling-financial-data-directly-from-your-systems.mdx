...
date: 2026-01-03
excerpt: "What is an API? A plain-English guide for accountants: API keys, endpoints, JSON responses in Python, and keeping credentials safe in .env files."
tags:
  - "Essentials"
...
**Q: Do accountants need to learn APIs?**
If you export a CSV, clean it in Excel, and upload it somewhere else every month — yes. APIs replace that loop with a script that pulls the same data on demand, and they're the foundation for every automation that follows: journal entries, reconciliations, and system-to-system syncs.

**Q: What is JSON in simple terms?**
JSON is the structured text format most APIs use to return data — essentially a nested dictionary of field names and values. In Python, a JSON response becomes a regular dictionary (or a list of dictionaries), so if you can work with those, you can read any API response.

**Q: Is it safe to use API keys?**
Yes, if you treat them like passwords: store them in a `.env` file that never gets committed to GitHub, load them with `python-dotenv`, and start with read-only permissions. The unsafe version is hardcoding keys in scripts you share or upload.
