...
Yes — NetSuite's REST Web Services expose most core records (customers, vendors, transactions, journal entries) through standard CRUD operations. Authentication uses Token-Based Authentication (OAuth1) with a Consumer Key/Secret and Token ID/Secret from an Integration Record.

**Q: What's the difference between Realm ID and Company ID in NetSuite?**
They represent the same account, formatted differently: the Realm ID (e.g. `9999999_SB1`) is used for authentication, while the Company ID (e.g. `9999999-sb1`) is used in API URLs. To convert, replace the underscore with a hyphen and lowercase everything — using the wrong one causes authentication or routing errors.

**Q: Can I create journal entries through the NetSuite API?**
Yes — journal entries are records like any other, created with a `POST` and a payload of line items. That's the foundation for automations like posting accruals from vendor billing data or syncing payroll entries from non-integrated systems.
